...
def get_scores(league,team_filter=None):
        ...
        #url = "http://www.fuerstjh.com/test.html"
                ...
                for i in range(1,len(data)):
                        ...
                        if (' at ' not in score):
                                teams = score.split('  ')
                                team1_name = teams[0][0:teams[0].rfind(' ')].lstrip(STRIP2)
                                team2_name = teams[1][0:teams[1].rfind(' ')].lstrip(STRIP2)
                                team1_score = teams[0][teams[0].rfind(' ')+1:].strip()
                                team2_score = teams[1][teams[1].rfind(' ')+1:].strip()
                        else:
                                teams = score.split(' at ')
                                #print teams
                                team1_name = teams[0].lstrip(STRIP2)
                                #print team1_name
                                team2_name = teams[1].lstrip(STRIP2)
                                #print team2_name
